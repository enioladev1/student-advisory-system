...
<div class="page-header d-flex align-items-center gap-3">
    <a href="{{ route('advisor.messages.index') }}" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-1">
        ...
        <div>
            ...
            <p style="margin:2px 0 0;font-size:12.5px;color:#64748b;">
                @if($user->studentProfile)
                    {{ $user->studentProfile->matric_number }} &middot; {{ $user->studentProfile->department }}
                @endif
            </p>
        </div>
    </div>
...
<div class="card">
    ...
    <div style="border-top:1px solid #e2e8f0;padding:16px 20px;">
        <form method="POST" action="{{ route('advisor.messages.store') }}">
            @csrf
