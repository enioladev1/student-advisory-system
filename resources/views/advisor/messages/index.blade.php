    ...
    @if($conversations->isEmpty())
        <div class="card-body">
            <div class="empty-state">
                ...
                <p>Students will appear here once they send you a message.</p>
            </div>
        </div>
    @else
        @foreach($conversations as $otherId => $lastMsg)
            ...
            @endphp
            <a href="{{ route('advisor.messages.thread', $other->id) }}"
               ...
               onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='{{ $isUnread ? '#f8fbff' : 'transparent' }}'">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    @if($other->studentProfile)
                        <div style="font-size:11.5px;color:#94a3b8;margin-top:2px;">{{ $other->studentProfile->matric_number }} &middot; {{ $other->studentProfile->department }}</div>
                    @endif
                </div>
